  ...
  if (loading) {
    return (
      <div className="text-center py-12">
        <div className="text-gray-500 dark:text-gray-400">Loading storage info...</div>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="mb-8 bg-white dark:bg-gray-900 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <h2 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">Disk Usage</h2>

        <div className="space-y-4">
          <div>
            <div className="flex justify-between text-sm mb-1">
              <span className="text-gray-600 dark:text-gray-400">Used by podcasts</span>
          ...
          <div className="grid grid-cols-3 gap-4 pt-4 border-t dark:border-gray-700">
            <div>
              <div className="text-sm text-gray-500 dark:text-gray-400">Total Used</div>
              <div className="text-lg font-semibold text-gray-900 dark:text-gray-100">
                {formatBytes(storage.total_used)}
              </div>
            </div>
            <div>
              <div className="text-sm text-gray-500 dark:text-gray-400">Free Space</div>
              <div className="text-lg font-semibold text-gray-900 dark:text-gray-100">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <h2 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">Storage by Feed</h2>

        {storage.feeds.length === 0 ? (
          <p className="text-gray-500 dark:text-gray-400 text-center py-4">No feeds yet.</p>
        ) : (
          <div className="space-y-4">
            {storage.feeds
              .sort((a, b) => b.total_size - a.total_size)
              .map((feed) => (
                <div key={feed.id} className="flex items-center justify-between py-2 border-b dark:border-gray-700 last:border-0">
      ...
      {/* Maintenance */}
      <div className="mt-8 bg-white dark:bg-gray-900 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div className="space-y-4">
          ...
          {migrationResult && (
            <div className={`p-4 rounded-md ${migrationResult.error ? 'bg-red-50 dark:bg-red-900/20' : 'bg-gray-50 dark:bg-gray-800'}`}>
              {migrationResult.error ? (
                <p className="text-sm text-red-700 dark:text-red-400">{migrationResult.error}</p>
              ) : (
                <div className="text-sm">
                  ...
                  {(migrationResult.thumbnails_downloaded > 0 || migrationResult.thumbnails_failed > 0) && (
                    <div className="mb-3 pb-3 border-b border-gray-200 dark:border-gray-700">
                      <p className="font-medium text-gray-700 dark:text-gray-300 mb-1">Thumbnail Downloads</p>
                      <ul className="space-y-1 text-gray-600 dark:text-gray-400">
                        ...
                        {migrationResult.thumbnails_failed > 0 && (
                          <li className="text-red-600 dark:text-red-400">Failed: {migrationResult.thumbnails_failed}</li>
                        )}
                      </ul>
                    </div>
                  )}
                  <p className="font-medium text-gray-700 dark:text-gray-300 mb-1">Letterboxing</p>
                  <ul className="space-y-1 text-gray-600 dark:text-gray-400">
                    ...
                    {migrationResult.failed > 0 && (
                      <li className="text-red-600 dark:text-red-400">Failed: {migrationResult.failed}</li>
                    )}
                  </ul>
